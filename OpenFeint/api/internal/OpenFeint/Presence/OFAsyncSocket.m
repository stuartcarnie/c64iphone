...
@interface OFAsyncSocket (Private)
 ...
 **/
@interface OFAsyncReadPacket : NSObject
{
@public
...
@implementation OFAsyncReadPacket
 ...
 **/
@interface OFAsyncWritePacket : NSObject
{
@public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation OFAsyncWritePacket
 ...
 **/
@interface OFAsyncSpecialPacket : NSObject
{
@public
...
@implementation OFAsyncSpecialPacket
...
- (void)doAcceptWithSocket:(CFSocketNativeHandle)newNative
{
	...
	OFAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (BOOL)setSocketFromStreamsAndReturnError:(NSError **)errPtr
{
	...
	if(sa->sa_family == AF_INET)
	{
		...
		//int yes = 1;
		//int setKeepAlive = setsockopt(native, SOL_SOCKET, SO_KEEPALIVE, &yes, sizeof(yes));
		//OFLog(@"setKeepAlive: %d", setKeepAlive);
	}
	else
	{
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	OFAsyncReadPacket *packet = [[OFAsyncReadPacket alloc] initWithData:buffer
															timeout:timeout
																tag:tag
												   readAllAvailable:NO
														 terminator:nil
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[OFAsyncSpecialPacket class]])
			{
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	OFAsyncWritePacket *packet = [[OFAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[OFAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)doSendBytes
{
	if((theCurrentWrite != nil) && (theWriteStream != NULL))
	{
		...
		while (!done && !error && CFWriteStreamCanAcceptBytes(theWriteStream))
		{
			...
			//OFLog(@"done? %d %d", done, bytesWritten);
		}
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	OFAsyncSpecialPacket *packet = [[OFAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	OFAsyncSocket *theSocket = [[(OFAsyncSocket *)pInfo retain] autorelease];
...
-(BOOL)enableKeepAlive {
	...
	int setKeepAlive = setsockopt(CFSocketGetNative(theSocket4), SOL_SOCKET, SO_KEEPALIVE, (void *)&setEnabledTo, sizeof(setEnabledTo));
	int gotKeepAliveSet = getsockopt(CFSocketGetNative(theSocket4), SOL_SOCKET, SO_KEEPALIVE, &enabled, &size);

	int setTimeout = setsockopt(CFSocketGetNative(theSocket4), IPPROTO_TCP, TCP_KEEPALIVE, (void *)&setTimeoutTo, sizeof(setTimeoutTo));
	int gotTimeoutSet = getsockopt(CFSocketGetNative(theSocket4), IPPROTO_TCP, TCP_KEEPALIVE, &timeout, &size);

	//OFLog(@"keepalive: %d,%d,%d timeout: %d,%d,%d", setKeepAlive, gotKeepAliveSet, enabled, setTimeout, gotTimeoutSet, timeout);

	return (setKeepAlive == 0) && (gotKeepAliveSet == 0) && (setTimeout == 0) && (gotTimeoutSet == 0);
}
