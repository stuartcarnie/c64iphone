...
+ (bool) localSetHighScore:(int64_t)score forLeaderboard:(NSString*)leaderboardId forUser:(NSString*)userId displayText:(NSString*)displayText customData:(NSString*)customData serverDate:(NSDate*)serverDate addToExisting:(BOOL) addToExisting
{
	...
	if (leaderboard && (!leaderboard.isAggregate || addToExisting))
	{
		...
		int64_t previousScore = [OFHighScoreService getPreviousHighScoreLocal:leaderboardId];
		if (addToExisting)
...
+ (void) sendPendingHighScores:(NSString*)userId silently:(BOOL)silently onSuccess:(const OFDelegate&)onSuccess onFailure:(const OFDelegate&)onFailure
{
	if ([OpenFeint isOnline] && userId != @"Invalid" && [userId longLongValue] > 0)
	{
		...
		for (sNullUserHighScoresQuery.execute(); !sNullUserHighScoresQuery.hasReachedEnd(); sNullUserHighScoresQuery.step())
		{
			int64_t nullUserScore = sNullUserHighScoresQuery.getInt64("score");
			NSString* leaderboardId = [NSString stringWithFormat:@"%d", sNullUserHighScoresQuery.getInt("leaderboard_id")];
			if( nullUserLeaderboard == nil || nullUserLeaderboard.resourceId != leaderboardId )
			{
				...
				leaderboardBestScore = [OFHighScoreService getPreviousHighScoreLocal:leaderboardId];
...
+ (int64_t) getPreviousHighScoreLocal:(NSString*)leaderboardId
{
	OFLeaderboard_Sync* leaderboard = [OFLeaderboardService getLeaderboardDetails:leaderboardId];
	...
	int64_t previousScore = (!sGetHighScoresQuery.hasReachedEnd() ? sGetHighScoresQuery.getInt64("score") : 0);
