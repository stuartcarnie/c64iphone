...
- (void)toggleSelectionOfUser:(OFUser*)user
{
	...
	OFUserCell* userCell = (OFUserCell*)[self.tableView cellForRowAtIndexPath:indexPath];
	OFUser* userInTable = (OFUser*)[self getResourceAtIndexPath:indexPath]; 
	if (userInTable)
	{
		if ([mSelectedUsers containsObject:userInTable])
		{
			userCell.checkedImageView.hidden = YES;
			[mSelectedUsers removeObject:userInTable];
		}
		else
		{
			userCell.checkedImageView.hidden = NO;
...
{
	[[cell checkedImageView] setHidden:![mSelectedUsers containsObject:user]];
}
